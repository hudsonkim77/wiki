...
- **통합 데이터 모델 ERD(2026-07-12 신설)**: 경영관리 업무보고 목록의 WBS 링크 아래에 "ERD:" 링크를 추가. 13개 ITIL practice 도메인 + CI_HISTORY까지 총 26개 테이블 전체 컬럼을 논리/물리 두 모델로 보여주는 Mermaid.js(CDN) 다이어그램이며, 토글 버튼으로 전환합니다. 정적 HTML은 `ERD.html`(새 탭 링크), Streamlit은 사이드바 "ERD" 페이지(`pages/16_ERD.py`, `st.page_link`로 연결)로 접근합니다.
