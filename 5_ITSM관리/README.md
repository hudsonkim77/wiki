...
- **좌측 사이드바(Streamlit, 2026-07-11 폴더 구조와 정합화)**: `st.navigation()`+`st.Page()`로 wiki 저장소의 도메인 폴더 순서(1·2·3·6~15)와 1:1로 맞춘 15개 항목을 구성 — ITSM관리(홈)·변경관리·장애관리·구성관리·문제관리·배포관리·요청관리·서비스수준관리·형상관리·운영상태관리·이벤트관리·테스트관리·연계관리·백업관리·경영관리·ERD(업무보고·ERD는 practice가 아니라 맨 끝에 유지). 메인 스크립트는 여전히 `홈.py`(Streamlit Cloud "Main file path" 설정 유지)이지만 사이드바 라벨은 "ITSM관리"로 표시됩니다.
- **전체 관리 페이지 CRUD(2026-07-12, `CHG_20260712_004`)**: 변경관리에서 처음 만든 패턴(상태/결과 선택 → 목록 조회, 하단에 등록/삭제)을 나머지 13개 practice 전부로 확장 — 장애관리(placeholder였음)와 6~15번 10개 도메인(문제~백업관리, 읽기 전용 조회였음) 모두 각자의 CSV를 직접 읽고 쓰는 완전한 CRUD가 됐습니다(3_구성관리만 카테고리 선택 방식으로 예외). 도메인 간 FK(예: 장애의 원인 변경, 이벤트의 발생 CI, 백업의 복원 원본)는 대상 CSV의 실제 ID를 selectbox로 불러와 연결합니다. 공통 로직은 `crud_helpers.py`(로드/저장/ID채번/FK조회)·`column_labels.py`(전체 컬럼 한글 라벨)로 모아 11개 페이지가 중복 없이 재사용합니다.
- **통합 데이터 모델 ERD(2026-07-12 신설)**: 경영관리 업무보고 목록의 WBS 링크 아래에 "ERD:" 링크를 추가. 13개 ITIL practice 도메인 + CI_HISTORY까지 총 26개 테이블 전체 컬럼을 논리/물리 두 모델로 보여주는 Mermaid.js(CDN) 다이어그램이며, 토글 버튼으로 전환합니다. 정적 HTML은 `ERD.html`(새 탭 링크), Streamlit은 사이드바 "ERD" 페이지(`pages/16_ERD.py`, `st.page_link`로 연결)로 접근합니다. 페이지는 본문 폭 제한 없이 화면 끝까지 사용하고, 컬럼명은 "한글명(ENGLISH_NAME)" 형식으로 표시하며, 마우스 휠 확대/축소·드래그 이동이 가능한 인터랙티브 캔버스로 구현되어 있습니다(`CHG_20260712_002`).
...
| `경영관리.html` | (정적 HTML 버전) 업무보고 5건을 제목+링크로 나열. 홈 대시보드 하단에 있던 패널을 별도 메뉴 페이지로 분리(`CHG_20260711_014`). WBS 링크 아래 ERD 링크 추가(`CHG_20260712_001`) |
| `ERD.html` | (정적 HTML 버전, 2026-07-12 신설) 통합 데이터 모델 ERD. Mermaid.js(CDN)로 논리/물리 두 모델을 그리고 버튼으로 토글. 13개 도메인+CI_HISTORY 총 26개 테이블 전체 컬럼 반영(`CHG_20260712_001`, `CFG_WEB_024`) |
...
| `홈.py` | (Streamlit 버전) 앱 진입점. `st.navigation()`+`st.Page()`로 15개 사이드바 항목(ITSM관리·1·2·3·6~15번 도메인·경영관리·ERD, 위 "화면 구성" 참고)을 등록하고 `nav.run()`으로 라우팅. 메인 화면 자체는 `render_home()` 함수(pandas로 CI/CHANGE/INCIDENT CSV를 직접 읽어 총 자산수·카테고리별 막대바(Altair)·스코어보드를 매번 최신값으로 렌더링, 하드코딩 스냅샷 아님)이며, 사이드바 라벨을 "ITSM관리"로 표시하기 위해 파일명(`홈.py`, Streamlit Cloud "Main file path" 설정과 일치해야 함)은 그대로 두고 이 함수만 명명함 |
| `pages/1_변경관리.py` | (Streamlit 버전) 실제 변경요청 등록/조회 화면 — 상태 선택(selectbox) → 목록 조회(`st.dataframe`), 변경요청 추가(폼)/삭제(선택+확인) 기능이 `1_변경관리/CHANGE.csv`를 직접 읽고 씀(구성관리 CRUD와 동일 패턴). 장애 대응으로 촉발된 변경은 `TRIGGERED_BY_INCIDENT_ID`로 `INCIDENT.csv`의 실제 장애를 선택해 연결 가능. 조회 표 헤더는 `column_labels.ko_labels()`로 한글 치환(원본 CSV 컬럼명/데이터는 그대로, 표시만 변경). 나머지 CRUD 페이지 전체가 따르는 원형(prototype). `CHG_20260711_019`·`CHG_20260712_003`·`004`로 변경 이력 등록 |
| `pages/2_장애관리.py` | (Streamlit 버전, 2026-07-12 CRUD 구현) 상태 선택 → 조회, 장애 등록(폼)/삭제. `CAUSED_BY_CHG_TICKET_ID`는 `CHANGE.csv` 실제 변경 티켓을 선택해 연결. `2_장애관리/INCIDENT.csv` 직접 수정. `CHG_20260712_004`로 변경 이력 등록 |
| `pages/4_경영관리.py` | (Streamlit 버전) `index.html`이었으면 홈 하단에 있었을 업무보고 5건 링크를 별도 사이드바 페이지로 분리. 정적 HTML의 `경영관리.html`과 동일 목록(수동 동기화, 자동 연동 아님). 사이드바 순서상 practice가 아니라 맨 끝에 위치. WBS 링크 아래 `st.page_link`로 ERD 페이지 연결 추가(`CHG_20260712_001`) |
| `pages/16_ERD.py` | (Streamlit 버전, 2026-07-12 신설) `ERD.html`의 패널 섹션을 그대로 잘라내 `st.components.v1.html`로 임베딩(구성관리와 동일하게 정적 HTML을 유일 원본으로 유지, Mermaid 다이어그램 정의를 중복 관리하지 않음) |
| `pages/6_문제관리.py` ~ `pages/15_백업관리.py` (10개) | (Streamlit 버전, 2026-07-12 CRUD 구현) 각 도메인 CSV(PROBLEM/DEPLOY/REQUEST/SLA/BASELINE/OPS_STATUS/EVENT/TEST_CASE/INTERFACE/BACKUP)를 직접 읽고 쓰는 등록/조회/삭제 화면. 상태(또는 결과) 선택 필터 → 조회 표 → 하단 등록/삭제, 도메인별 README 필드정의에 맞춘 ENUM selectbox, 타 도메인 참조 FK는 대상 CSV의 실제 ID를 selectbox로 연결(예: 이벤트관리의 CI_ID/SOURCE_OPS_ID/ESCALATED_INCIDENT_ID, 백업관리의 자기참조 RESTORED_FROM_BACKUP_ID). 2026-07-11에 신설했던 읽기 전용 `domain_view.render_domain_page()` 래퍼를 대체(해당 모듈은 삭제). `CHG_20260712_004`로 변경 이력 등록 |
| `pages/3_구성관리.py` | (Streamlit 버전) 상단은 `구성관리.html`의 "자산 배치도(3D)"·"데이터 모델 ERD" 두 패널을 `st.components.v1.html`(iframe)로 그대로 임베딩(마크업/CSS는 `구성관리.html`+`style.css`를 실행 시점에 읽어와 재사용, 정적 버전과 어긋날 일 없음). 하단은 실제 CI 등록/조회 화면 — 카테고리 선택(selectbox) → 목록 조회(`st.dataframe`), CI 추가(폼)/삭제(선택+확인) 기능을 구현. `3_구성관리/CI.csv`를 직접 읽고 쓰며, 추가/삭제 이력은 `3_구성관리/구성이력/CI_HISTORY.csv`에 기록됨. **정적 HTML 버전에는 없음(Streamlit 전용 기능)**. 다른 CRUD 페이지와 달리 카테고리 선택 방식을 유지(도메인 특성상 상태값보다 카테고리 분류가 더 자연스러움) |
...
| `column_labels.py` | (Streamlit 버전, 2026-07-12 신설) 전체 13개 도메인 CSV의 컬럼명 → 한글 라벨 딕셔너리(`COLUMN_LABELS_KO`, ERD 작업의 "한글명(영문명)" 번역을 재사용) + `ko_labels(columns)` 헬퍼. 11개 CRUD 페이지가 조회 표 헤더를 한글로 표시할 때 공통으로 import |
| `crud_helpers.py` | (Streamlit 버전, 2026-07-12 신설) 11개 CRUD 페이지가 공유하는 보일러플레이트 — `load_df`/`save_df`(CSV 읽기/쓰기), `next_id`(`{PREFIX}_{YYYYMMDD}_{순번}` 채번), `fk_options`(다른 도메인 CSV의 ID 목록을 FK 선택지로 조회). 폼 위젯 구성 자체는 도메인마다 달라 각 페이지 파일에 그대로 둠 |
...
| `CFG_WEB_024` | `ERD.html` | L2_PAGE (부모: `CFG_WEB_019`) |

최초 구축 변경 티켓: `CHG_20260711_001` ([1_변경관리/CHANGE.csv](../1_변경관리/CHANGE.csv)), 연결 매핑: `MAP_0061`~`MAP_0064` ([변경히스토리/CHANGE_CI_MAP.csv](../1_변경관리/변경히스토리/CHANGE_CI_MAP.csv)). `CFG_WEB_023`은 `CHG_20260711_014`·`MAP_0065`로 추가 등록. `CFG_WEB_024`(ERD.html)는 `CHG_20260712_001`·`MAP_0066`로 추가 등록
...
| 2026-07-12 | 경영관리 업무보고 목록의 WBS 링크 아래에 "ERD:" 링크 신설. 13개 ITIL practice 도메인 + CI_HISTORY까지 총 26개 테이블(마스터 13·연결테이블 12·CI_HISTORY 1) 전체 컬럼을 논리/물리 두 모델로 표시하는 `ERD.html`을 Mermaid.js(CDN, 사전 사용자 승인)로 신규 작성, 토글 버튼으로 전환. Streamlit은 `pages/16_ERD.py`가 `ERD.html`의 다이어그램 정의를 그대로 재사용(components.html 임베딩, 정적 HTML을 유일 원본으로 유지)하고 `pages/4_경영관리.py`에는 `st.page_link`로 연결. 개발 중 playwright 헤드리스 브라우저로 (1) Mermaid가 `display:none` 요소의 자기참조 엣지(CI-CI, BACKUP-BACKUP)를 NaN 좌표로 깨뜨리는 문제 → `visibility:hidden` 토글로 수정, (2) `svg width="100%"`로 인해 26개 테이블 물리 모델이 읽을 수 없이 축소되는 문제 → viewBox 기준 네이티브 픽셀 크기 강제 지정으로 수정. `CHG_20260712_001`로 변경 이력 등록, `CFG_WEB_024`·`MAP_0066`로 CI 연결 | Claude |
| 2026-07-12 | ERD 화면이 좁게 보인다는 피드백으로 페이지 전용 클래스(`erd-wide-main`)로 본문 폭 제한(1120px)을 풀어 화면 끝까지 사용. 전체 컬럼명을 Mermaid attribute comment로 "한글명(ENGLISH_NAME)" 형식으로 표기(90여개 고유 컬럼, 번역 딕셔너리는 Python 스크립트로 생성). 글자 크기를 줄이는 대신 뷰포트(70vh)+CSS transform 기반 마우스 휠 확대/축소·드래그 이동 인터랙션을 추가해 한눈에 보기와 확대 보기를 모두 지원(정적 HTML·Streamlit 동일 적용). `CHG_20260712_002`로 변경 이력 등록 | Claude |
| 2026-07-12 | 변경요청 조회 표(`pages/1_변경관리.py`)의 컬럼 헤더(`CHG_TICKET_ID` 등)가 원본 CSV 컬럼명 그대로 노출되던 것을 `COLUMN_LABELS_KO` 딕셔너리로 한글 라벨(예: 변경 티켓 ID)로 치환. `rename(columns=...)`으로 표시만 바꾸고 `CHANGE.csv` 원본 컬럼명/데이터는 그대로 유지. `CHG_20260712_003`로 변경 이력 등록 | Claude |
| 2026-07-12 | 변경관리 CRUD 화면(상태 선택→조회, 등록/삭제) 패턴을 전체 관리 페이지로 확장. `pages/2_장애관리.py`(placeholder였음)와 `pages/6_문제관리.py`~`15_백업관리.py` 10개 페이지(읽기 전용 `domain_view.render_domain_page()`였음) 전부를 각 도메인 CSV를 직접 읽고 쓰는 완전한 CRUD로 교체 — 도메인별 README 필드정의에 맞춘 ENUM selectbox 구성, 타 도메인 참조 FK(장애의 원인 변경, 이벤트의 발생 CI 등, 백업의 자기참조 복원 원본 포함)는 대상 CSV의 실제 ID를 selectbox로 연결. 반복 로직은 `crud_helpers.py`(로드/저장/ID채번/FK조회), 전체 컬럼 한글 라벨은 `column_labels.py`(ERD 작업의 번역 재사용)로 공통화해 11개 페이지 중복을 없앰. 더 이상 쓰이지 않는 `domain_view.py`는 삭제. `streamlit.testing.v1.AppTest`로 15개 페이지 전체 무예외 확인 후 요청관리·이벤트관리·백업관리(자기참조 FK)·장애관리 4개는 실제 등록→CSV반영→삭제 왕복까지 자동화 테스트로 검증(테스트 데이터는 검증 후 삭제해 원상 복구), 실제 서버+playwright로 11개 페이지 전체 한글 헤더 렌더링과 무에러를 확인. `CHG_20260712_004`로 변경 이력 등록 | Claude |
| 2026-07-12 | 자산관리 전체 312건(인력 30·응용 24·인프라 258) 대상 통합 초기 베이스라인 v1.0(`BSL_20260712_002`) 확정 작업으로 `BASELINE_CI_MAP` 스키마에 `BASELINE_VERSION`·`BASELINE_AREA`·`APPLIED_DT` 3개 컬럼 추가, `ERD.html`(물리 모델)·`column_labels.py`에 반영. 경영관리 페이지(정적 HTML·Streamlit)에 구축산출물 "초기 베이스라인 정의서 v1.0"(ITSM-BSL-2026-001) 링크 추가. 저장소 루트 `requirements.txt`에 버전 고정이 없던 것을 발견해 `streamlit==1.50.0`·`pandas==2.3.3`으로 고정. `CHG_20260712_008`로 변경 이력 등록 | Claude |
| 2026-07-12 | 초기 베이스라인 정의서 v1.1(DDL 개정판) 발행 — v1.0의 서술형 DDL 표를 실행 가능한 SQL(CREATE TABLE/ALTER TABLE, PK·FK·CHECK·UNIQUE 제약조건)로 구체화하고 목차·ERD 관계도(ASCII)·공단 직인 날인란 추가. v1.0 원본은 보존, 별도 파일로 신규 발행. 경영관리 페이지 링크 추가. `CHG_20260712_009`로 변경 이력 등록 | Claude |
