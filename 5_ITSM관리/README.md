...
| `assets/logo-mark.png`, `assets/logo-full.png` | (정적 HTML 버전) `../RAW/logo.jpeg`(AI활성화진흥공단 로고)를 Pillow로 배경 제거해 만든 투명 PNG. `nav.js`가 상단 메뉴바 브랜드 옆(`logo-mark`)과 footer(`logo-full`)에 자동 삽입 |
| `assets/logo-mark-flat.png`, `assets/logo-full-flat.png` | (Streamlit 버전) 위와 동일한 크롭에 원본 크림색 배경을 남긴 버전. `st.logo()`는 투명 배경을 지원하지 않아 별도 제작(다크 테마에서도 로고 원본 배색이 그대로 보이도록) |
