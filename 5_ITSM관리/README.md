...
| 최근 변경 요약 | `../1_변경관리/CHANGE.csv` 마지막 행 | 현황 스코어보드 하단에 가장 최근 변경 티켓의 제목/상세를 노출(`data.js`의 `latestChange`, `홈.py`는 `change_df.iloc[-1]`로 매번 계산) |
...
## 구성관리 페이지(`구성관리.html`) 구성
`구성관리.html`은 더 이상 순수 placeholder가 아니라, 아래 두 시각 자료가 상단/하단에 배치되어 있습니다(CI 등록/조회 입력 화면 자체는 여전히 설계 전).
