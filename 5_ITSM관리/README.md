...
## 화면 구성
- **상단 메뉴바**: 모든 페이지(`index.html`/`변경관리.html`/`장애관리.html`/`구성관리.html`/`경영관리.html`)에 동일하게 떠 있는 공용 네비게이션. ITSM관리·변경관리·장애관리·구성관리·경영관리 5개 메뉴 버튼이 각 페이지로 실제 이동하며, 현재 페이지 버튼은 활성 표시됩니다. Streamlit 버전은 `st.navigation()`+`st.Page()`로 동일한 5개 항목을 사이드바("좌측 메뉴")에 구성 — 메인 스크립트는 여전히 `홈.py`(Streamlit Cloud "Main file path" 설정 유지) 파일이지만 사이드바 라벨은 "ITSM관리"로 표시됩니다. 변경관리/장애관리/구성관리(CI 등록·삭제 제외)의 **입력 화면 내용물**은 아직 설계 전이라 "설계 전" 안내 문구만 있는 placeholder 상태입니다.
...
| `pages/1_변경관리.py`, `pages/2_장애관리.py` | (Streamlit 버전) `st.navigation()`으로 사이드바에 등록되며 현재는 "입력 화면은 추후 설계 예정" placeholder만 있음 |
| `pages/4_경영관리.py` | (Streamlit 버전) `index.html`이었으면 홈 하단에 있었을 업무보고 5건 링크를 별도 사이드바 페이지로 분리. 정적 HTML의 `경영관리.html`과 동일 목록(수동 동기화, 자동 연동 아님) |
