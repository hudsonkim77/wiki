...
| 파일 위치 | 설명 | 건수 |
|---|---|---|
| `INCIDENT.csv` (이 폴더) | 위 필드 정의를 구현한 장애 마스터 테이블(헤더만) | 0건 — 원본 데이터에 장애 이력이 없어 스키마만 우선 생성. 장애 발생 시 이 표에 행을 추가 |
| [`장애이력/INCIDENT_CI_MAP.csv`](장애이력/README.md) | INCIDENT-CI 중간(연결) 테이블(헤더만) | 0건 — 위와 동일한 사유 |
