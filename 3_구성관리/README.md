...
| 파일명 | 설명 | 출처 | 최종 수정일 | 비고 |
|---|---|---|---|---|
| CFG.csv | CI(자산) 마스터. 서버/네트워크장비/PC/스토리지/웹페이지/인력(HUMAN) 등 구성항목과 계층(부모-자식), 담당자, 최근 변경티켓 정보를 포함 | 자산 실사/CMDB 추출 | 2026-07-11 | 중복 제거 완료(고유 CI 306건, 헤더 포함 307행). 원본은 `20260711_CFG_backup.csv`로 보관 |
| 20260711_CFG_backup.csv | CFG.csv 중복 제거 이전 원본 백업(564행, 헤더 2회 포함, 중복 행 다수) | CFG.csv 정제 전 스냅샷 | 2026-07-11 | 참고/복구용, 직접 사용 금지 |
