...
| CI.csv (이 폴더) | 아래 "CI(자산) 마스터 필드 정의"를 실제 테이블로 구현한 결과물. `../RAW/CFG.csv`(306건)에 [5_ITSM관리](../5_ITSM관리/README.md) 통합관리대시보드 자체를 CI로 등록한 4건(`CFG_WEB_019`~`022`)을 더해 총 310건 | `../RAW/CFG.csv` + 자체 등록 | 2026-07-11 | 이 폴더의 개념 설계를 그대로 반영한 산출 테이블. `pages/3_구성관리.py`의 CI 등록/삭제 화면을 통해서도 직접 수정됨(310건은 스냅샷, 실사용 중 변동 가능) |
